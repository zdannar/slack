...
import (
	...
	"github.com/zdannar/slack/internal/errorsx"
	"github.com/zdannar/slack/internal/timex"
)
...
func (rtm *RTM) handleEvent(typeStr string, event json.RawMessage) {
	...
	if err != nil {
		rtm.Debugf("RTM Error, could not unmarshall event %q: %s : %s\n", typeStr, string(event), err)
